...
const colorMap: Record<string, { bg: string; text: string; activeBg: string; border: string }> = {
  blue: {
    ...
    activeBg: "bg-blue-50 dark:bg-gray-700",
    border: "border-blue-500",
  },
  purple: {
    bg: "bg-purple-100 dark:bg-purple-900/40",
    text: "text-purple-600 dark:text-purple-400",
    activeBg: "bg-purple-50 dark:bg-gray-700",
    border: "border-purple-500",
  },
  amber: {
    bg: "bg-amber-100 dark:bg-amber-900/40",
    text: "text-amber-600 dark:text-amber-400",
    activeBg: "bg-amber-50 dark:bg-gray-700",
    border: "border-amber-500",
  },
  teal: {
    bg: "bg-teal-100 dark:bg-teal-900/40",
    text: "text-teal-600 dark:text-teal-400",
    activeBg: "bg-teal-50 dark:bg-gray-700",
    border: "border-teal-500",
  },
  pink: {
    bg: "bg-pink-100 dark:bg-pink-900/40",
    text: "text-pink-600 dark:text-pink-400",
    activeBg: "bg-pink-50 dark:bg-gray-700",
    border: "border-pink-500",
  },
  gray: {
    bg: "bg-gray-200 dark:bg-gray-700",
    text: "text-gray-600 dark:text-gray-400",
    activeBg: "bg-gray-50 dark:bg-gray-700",
    border: "border-gray-500",
  },
  indigo: {
    bg: "bg-indigo-100 dark:bg-indigo-900/40",
    text: "text-indigo-600 dark:text-indigo-400",
    activeBg: "bg-indigo-50 dark:bg-gray-700",
...
export function TabLayout({ tabs, defaultTab, activeTab, onTabChange }: TabLayoutProps) {
  const [internalActiveKey, setInternalActiveKey] = useState(defaultTab ?? tabs[0]?.key ?? "");
  const currentActiveKey = activeTab ?? internalActiveKey;
  const activeItem = tabs.find((tab) => tab.key === currentActiveKey) ?? tabs[0];

  useEffect(() => {
    if (activeTab !== undefined) return;
    setInternalActiveKey(defaultTab ?? tabs[0]?.key ?? "");
  ...
  return (
    <div>
        ...
        <div className="relative">
          {activeItem && (
            <div className="pointer-events-none absolute left-3 top-1/2 flex -translate-y-1/2 items-center gap-2">
              <activeItem.icon className="h-4 w-4 text-gray-500" />
            </div>
          )}
          <select
      ...
      <div className="hidden md:block">
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <div className="flex gap-1 p-1.5">
            {tabs.map((tab) => {
              const isActive = tab.key === activeItem?.key;
      ...
      <div className="mt-4">{activeItem?.component}</div>
    </div>
  );
}
