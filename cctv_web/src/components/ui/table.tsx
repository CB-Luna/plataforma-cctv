...
function TableHeader({ className, style, ...props }: React.ComponentProps<"thead">) {
  return (
    <thead
      ...
      style={{ backgroundColor: "var(--table-header-bg)", ...style }}
      {...props}
    />
  )
}
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b transition-colors even:bg-[var(--table-stripe-bg,transparent)] hover:bg-[var(--table-hover-bg)] has-aria-expanded:bg-muted/50 data-[state=selected]:bg-muted",
        className
      )}
      {...props}
    />
